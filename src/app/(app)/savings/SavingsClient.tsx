...
export default function SavingsClient({ initialGoals, householdId }: Props) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    if (!error && data) {
      ...
      setShowModal(false);
      setForm({ name: "", target_amount: "", color: COLORS[0], icon: GOAL_ICONS[0], deadline: "" });
    }
    setSaving(false);
    router.refresh();
  }
  ...
  async function handleDeposit(e: React.FormEvent) {
    ...
    if (!error) {
      setGoals(prev => prev.map(g => g.id === depositModal.id ? { ...g, current_amount: newAmount } : g));
  ...
  return (
    <div className="px-4 pt-6 max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-2xl font-bold text-gray-900">Sparmål</h1>
        {householdId && (
          <button
            ...
            return (
              <div key={goal.id} className="bg-white rounded-2xl p-5 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {done && <span className="text-xs bg-green-100 text-green-600 rounded-full px-2 py-0.5">Klart! 🎉</span>}
                      {daysLeft !== null && !done && (
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-400">{Math.round(pct)}% av målet</span>
                  {!done && (
                    <button
                      onClick={() => setDepositModal(goal)}
                      className="text-sm font-medium text-green-600 bg-green-50 rounded-lg px-3 py-1.5"
                    >
                      + Sätt in
                    </button>
                  )}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold">Nytt sparmål</h2>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="T.ex. Semester i Thailand"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Målbelopp (SEK)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Deadline (valfritt)</label>
                <input
                  type="date"
                  value={form.deadline}
                ...
                <div className="grid grid-cols-6 gap-2">
                  {GOAL_ICONS.map(icon => (
                    <button
                      ...
                      className={`text-2xl p-2 rounded-xl ${form.icon === icon ? "bg-green-100 ring-2 ring-green-400" : "hover:bg-gray-100"}`}
                    >
                      {icon}
                    </button>
                  ))}
      ...
      {depositModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold">Sätt in pengar</h2>
              <button onClick={() => setDepositModal(null)} className="text-gray-400 text-2xl">&times;</button>
            </div>
            <p className="text-sm text-gray-500 mb-4">
              {depositModal.icon} {depositModal.name} · Kvar:{" "}
              {formatCurrency(depositModal.target_amount - depositModal.current_amount)}
            </p>
